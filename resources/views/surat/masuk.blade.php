...
<div class="container-fluid animate__animated animate__fadeIn">
    <div class="row mb-4">
        <div class="col-12">
            <div class="card p-4 border-0 shadow-sm text-white" style="background: var(--stmc-gradient); border-radius: 20px;">
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $s->nama_surat }}</td>
                            <td>{{ $s->tanggal_surat ? \Carbon\Carbon::parse($s->tanggal_surat)->translatedFormat('d F Y') : '-' }}</td>
                            <td><small class="text-muted">{{ $s->user->nama_lengkap ?? 'Admin' }}</small></td>
                            <td class="text-center">
                                <a href="{{ route('surat.show', $s->id) }}" class="btn btn-sm btn-outline-primary border-0">
